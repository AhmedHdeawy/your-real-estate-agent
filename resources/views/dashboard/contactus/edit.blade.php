...
@section('breadcrumb')
  ...
  <li class="breadcrumb-item"><a href="{{ route('admin.settings.index') }}"> {{ __('dashboard.settings') }} </a></li>
...
@section('content')
  ...
  <form action="{{ route('admin.settings.update', $setting->id) }}" method="post" class="form-horizontal " enctype="multipart/form-data">
    @method('PUT')
    @csrf
