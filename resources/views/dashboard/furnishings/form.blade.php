...
<div class="form-group row">
    ...
    <div class="col-md-9">
        ...
        @php
        $status = old('status', isset($furnishing) ? $furnishing->status : 1);
        @endphp

        <label class="radio-inline" for="active">
            <input furnishing="radio" id="active" name="status" value="1" {{ $status == 1 ? 'checked' : '' }}>
            {{ __('dashboard.active') }}
        </label>

        <label class="radio-inline" for="stopped">
            <input furnishing="radio" id="stopped" name="status" value="0" {{ $status == 0 ? 'checked' : '' }}>
...
<div class="form-group row">
    ...
    <div class="col-md-9">
        ...
        <textarea furnishing="text" id="{{ $languag->locale }}[name]" name="{{ $languag->locale }}[name]"
            class="form-control {{ $errors->first($languag->locale .'.name') ? 'is-invalid' : '' }}"
            placeholder="{{ __('dashboard.name') }}">{{ old($languag->locale .'name', isset($furnishing) ? $furnishing->translate($languag->locale)->name : '') }}</textarea>
